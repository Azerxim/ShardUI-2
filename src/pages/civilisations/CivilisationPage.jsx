...
import { gouvernementModal } from '@/config/modals/gouvernement';
import { civilisationMemberModal } from '@/config/modals/civilisation-member';
import { livreModal } from '@/config/modals/livre';
import { villeModal } from '@/config/modals/ville';
import {
    getCivilisationById,
...
export default function CivilisationPage() {
    ...
    const FctModify = [
        ...
        { id: 2, title: "Modifier", icon: "fas fa-pen", class: "bg-base-200 hover:bg-base-300", connected: true, authorisation: auth, function: () => showModal(civilisationModal, "edit") }
    ];

    const FctGouvernement = [
        { id: 1, title: "Modifier", icon: "fas fa-pen", class: "bg-base-200 hover:bg-base-300", connected: true, authorisation: auth, function: () => showModal(gouvernementModal, "edit") }
    ];

    const FctMembers = [
        { id: 1, title: "Ajouter", icon: "fas fa-plus", class: "bg-base-200 hover:bg-base-300", connected: true, authorisation: auth, function: () => showModal(civilisationMemberModal, "add") }
    ];

    const FctVilles = [
        { id: 1, title: "Nouveau", icon: "fas fa-plus", class: "bg-base-200 hover:bg-base-300", connected: true, authorisation: auth, function: () => showModal(villeModal, "add") }
    ];

    const FctLivres = [
        { id: 1, title: "Nouveau", icon: "fas fa-plus", class: "bg-base-200 hover:bg-base-300", connected: true, authorisation: auth, function: () => showModal(livreModal, "add") }
    ];
        ...
        <>
            ...
            <main className="container mx-auto p-4">
                <div className="flex flex-col items-center justify-center gap-2">
                    ...
                    <DynamicModal config={civilisationModal} mode="edit" onSubmit={updateCivilisation} onDelete={() => navigate('/civilisations')} />
                    <DynamicModal config={civilisationMemberModal} mode="add" onSubmit={addMember} />
                    <TransferFounderModal id={TRANSFER_MODAL_ID} entity="civilisation" entityId={id} members={members} onTransfer={updateMembers} />
                    <DynamicModal config={gouvernementModal} mode="edit" onSubmit={updateGouvernement} onDelete={() => setGouvernement(null)} />

                    <DynamicModal config={livreModal} mode="add" onSubmit={(livre) => setLivres((prev) => [...prev, livre])} />
                    <DynamicModal config={villeModal} mode="add" onSubmit={(ville) => setVilles((prev) => [...prev, ville])} />
                </div>
            </main>
        </>
    );
